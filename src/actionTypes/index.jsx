// カウンター増減
export const INCREMENT = 'INCREMENT';
export const DECREMENT = 'DECREMENT';
// ダイアログ
export const DIALOG_ACTION = 'DIALOG_ACTION';
// テキスト
export const TEXT_SET = 'TEXT_SET';
export const TEXT_ADD = 'TEXT_ADD';
export const TEXT_DELETE = 'TEXT_DELETE';
export const FORM_DELETE = 'FORM_DELETE';
export const NAME_SET = 'NAME_SET';
export const PROGRESS_TOGGLE = 'PROGRESS_TOGGLE';
// 電卓
export const INPUT_NUMBER = 'INPUT_NUMBER';
export const CALCUL_LOGIC = 'CALCUL_LOGIC';
export const DELETE_NUMBER = 'DELETE_NUMBER';
// ナビゲーションバー
export const INPUT_TEXT_TOGGLE = 'INPUT_TEXT_TOGGLE';
export const CALUCLOTOR_TOGGLE = 'CALUCLOTOR_TOGGLE';
export const GOTANDA_TOGGLE = 'GOTANDA_TOGGLE';
export const GOTANDA_DETAIL_TOGGLE = 'GOTANDA_DETAIL_TOGGLE';
// ボディ
export const BODY_TOGGLE = 'BODY_TOGGLE';

// 登録
export const RADIO_REGIST = 'RADIO_REGIST';
export const NAME_REGIST = 'NAME_REGIST';
export const HANDLE_NAME_REGIST = 'HANDLE_NAME_REGIST';
export const DATE_REGIST = 'DATE_REGIST';
export const COMMENT_REGIST = 'COMMENT_REGIST';
export const DELETE_REGIST = 'DELETE_REGIST';
export const PASSWORD_REGIST = 'PASSWORD_REGIST';
export const SEARCH_RESULT = 'SEARCH_RESULT';
export const INPUT_DETAIL_INFO = 'INPUT_DETAIL_INFO';

